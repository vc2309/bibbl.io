...
object_key_list = ["test_user/Extreme Economies-Notebook.html","test_user/Being Mortal-Notebook.html.html"]

for object_key in object_key_list:
    raw_notes = raw_notes_dao.get_notes_by_objkey(object_key)
    for item in raw_notes:
        raw_notes_dao.delete_item_by_id(item['note_id'])
    smart_notes = smart_notes_dao.get_notes_by_objkey(object_key)
    for item in smart_notes:
        smart_notes_dao.delete_item_by_id(item['smart_note_id'])
    snapshots = snap_shot_dao.get_snaps_by_userid('test_user')
